...
import com.woting.WTConstants;
import com.woting.common.model.Owner;
import com.woting.common.util.SessionUtils;
...
import com.woting.dictionary.model._OwnerDictionary;
...
public class DictSessionService implements SessionLoader {
    ...
    public void loader(HttpSession session) throws Exception {
        ...
        session.removeAttribute(WTConstants.SESSION_OWNER_DICT);
        session.setAttribute(WTConstants.SESSION_OWNER_DICT, _od);
        //启动加载线程
    ...
    public _OwnerDictionary loadcheckData(HttpSession session) throws Exception {
        _OwnerDictionary _od = (_OwnerDictionary)session.getAttribute(WTConstants.SESSION_OWNER_DICT);
        if (_od==null) {
            loader(session);
            _od = (_OwnerDictionary)session.getAttribute(WTConstants.SESSION_OWNER_DICT);
